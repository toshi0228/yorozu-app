...
import { withGuestLayout, withMemberLayout, isLogIn } from '../components/Layouts/RouteWithLayout'
...
const Router = (props) => {
  return (
    <ConnectedRouter history={props.history}>
      <Switch>
        {/* トップページ */}
        <Route exact path={routes.top()} component={withGuestLayout(ProfileListPage)} />

        {/* tokenがある時のトップページ */}
        <Route exact path={routes.memberTop()} component={withMemberLayout(requireAuth(ProfileListPage))} />

        {/* プロフィール詳細ページ */}
        <Route path={routes.profileDetail()} render={withGuestLayout(ProfileDetailPage)} />
        {/* <Route path={routes.profileDetail()} render={isLogIn(ProfileDetailPage)} /> */}
        ...
        {/* ボツページ（トップページ） */}
        <Route exact path={routes.contracting()} render={withMemberLayout(ContractingPage)} />
      </Switch>
    </ConnectedRouter>
  )
}
